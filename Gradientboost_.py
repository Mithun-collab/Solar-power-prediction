...
# Connect to MySQL
db = MySQLdb.connect(host="localhost", user="root", passwd="", db="prediction_v6")
cur = db.cursor()
...
while True:
    ...
    # Train models using Gradient Boosting
    model_temp = GradientBoostingRegressor(n_estimators=100, learning_rate=0.1, max_depth=4)
    model_ghi = GradientBoostingRegressor(n_estimators=100, learning_rate=0.1, max_depth=4)
    ...
    # Insert into DB
    sql = """INSERT INTO gradient_prediction (time_updated, Temperature, GHI, power) VALUES (%s, %s, %s, %s)"""
    try:
