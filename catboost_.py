...
# Connect to MySQL
db = MySQLdb.connect(host="localhost", user="root", passwd="", db="prediction_v7")
cur = db.cursor()
...
while True:
    ...
    # Train models using CatBoost
    model_temp = CatBoostRegressor(iterations=100, learning_rate=0.1, depth=4, verbose=0)
    model_ghi = CatBoostRegressor(iterations=100, learning_rate=0.1, depth=4, verbose=0)
    ...
    # Insert into DB
    sql = """INSERT INTO cat_prediction (time_updated, Temperature, GHI, power) VALUES (%s, %s, %s, %s)"""
    try:
