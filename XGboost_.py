...
# Connect to MySQL
db = MySQLdb.connect(host="localhost", user="root", passwd="", db="prediction_v4")
cur = db.cursor()
...
while True:
    ...
    model_temp = XGBRegressor(objective='reg:squarederror', n_estimators=100, learning_rate=0.1)
    model_ghi = XGBRegressor(objective='reg:squarederror', n_estimators=100, learning_rate=0.1)
    ...
    # Insert into DB
    sql = """INSERT INTO xg_prediction (time_updated, Temperature, GHI, power) VALUES (%s, %s, %s, %s)"""
    try:
